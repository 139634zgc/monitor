...
var crossTable = {
    table: function (args) {
        ...
        var html = "<table class = 'table table-striped coma-base-table fixed_headers' id='tableWrapper" + random + "'><thead>",
            colContent = "<tr>";
        ...
        html += colContent + "</tr><tr class='havePosition'></tr></thead><tbody>";
        ...
        var PaginationDom = "<div class='" + p_class + "'><div class='page'><div class='jumpPage'>跳至<input type='text' value=''><span>页</span></div><ul></ul></div></div>";
        ...
        this.pressKey(dataPage.length, pageObj, p_class);
    },
    ...
    pressKey: function (pageCount, pageObj, random) {
        var _this = this;
        $('.page .jumpPage input').keyup(function (e) {
            var pageNum = parseInt($(this).val());
            if (e.keyCode == 13 && pageNum) {
                if(pageNum>pageCount){return;}
                var dom = $(this).parent().siblings()[0];
                var tbody = $(e.target.offsetParent).find('tbody')[0];
    ...
    clickPageNum: function (data, chartConfig, drill, random, pageNum) {
        var _this = this;
        $('.' + random).on('click', 'a.pageLink', function (e) {
            var pageNum = pageNum ? pageNum - 1 : e.target.innerText - 1;
            var pageObj = {
                data: data,
                chartConfig: chartConfig,
                drill: drill
    ...
    renderPagination: function (pageCount, pageNumber, pageObj, target) {
        var liStr = '<li><a class="previewLink"><img src = "dist/img/ic_arrow_down_nor.png"></img></a></li>';
        if (pageCount < 10) {
        ...
        liStr += '<li><a class="nextLink"><img src = "dist/img/ic_arrow_down_nor.png"></img></a></li>';
        if (target) {
    ...
    clickNextPrev: function (pageCount, pageObj, random) {
        ...
        $('.' + random).on('click', '.page a.previewLink', function (e) {
            var kids = $(this).parent().siblings();
            var dom = this.parentNode.parentNode;
            var tbody = $(e.target.offsetParent).find('tbody')[0];
            for (var i = 0; i < kids.length; i++) {
                if ($(kids[i]).children()[0].className.indexOf('current') > -1) {
                    var pageNum = parseInt($(kids[i]).children()[0].innerText) - 1;
                }
            }
            tbody.innerHTML = _this.render(pageObj.data[pageNum - 1], pageObj.chartConfig, pageObj.drill);
            _this.renderPagination(pageCount, pageNum, pageObj, dom);
